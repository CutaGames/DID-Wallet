package com.tancheng.carbonchain.utils.dropdowntablayout.video;
...
public class VideoTabLayout extends TabLayout {
    ...
    public VideoTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public VideoTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            addTab(newTab().setText(titleList.get(i).get(0)));//默认=取第一个作为首次加载显示的tab名字
    ...
    public View getTabView(final int position, final List<String> titleList){
        final View view = LayoutInflater.from(mContext).inflate(R.layout.custom_tab_layout_video_item_tab, null);
