...
import com.tancheng.carbonchain.activities.asset.wallet.utils.AppFilePath;
...
public class ETHWalletUtil {
    ...
    public static ETHWallet generateMnemonic(String walletName, String pwd) {
        String[] pathArray = WalletType.ETH.getPath().split("/");
        String passphrase = "";
    ...
    public static ETHWallet generateWalletByMnemonic(String walletName, DeterministicSeed ds,
        ...
        if (ethWallet != null) {
            String mnemonic = convertMnemonicList(mnemonicList);
            ethWallet.setMnemonic(AESCrypt.encrypt(mnemonic,pwd));
        }
        return ethWallet;
    }
    ...
    private static ETHWallet generateWallet(String walletName, String pwd, ECKeyPair ecKeyPair) {
        ...
        ethWallet.setPassword(EncryptUtils.encryptMD5ToString(pwd));
        LogUtils.i("keystore 地址：" + destination.getAbsolutePath());
        return ethWallet;
    }
...
//            ToastUtils.showToast(R.string.load_wallet_by_official_wallet_keystore_input_tip);
    ...
    public static ETHWallet modifyPassword(long walletId, String oldPassword, String newPassword) {
        ETHWallet ethWallet = WalletDaoUtils.getIntence().getWalletInfo(walletId);
        Credentials credentials = null;
        ECKeyPair keypair = null;
        try {
            String walletName = ethWallet.getName();
            ...
            ethWallet.setPassword(EncryptUtils.encryptMD5ToString(newPassword));
            WalletDaoUtils.getIntence().updateWallet(ethWallet);
        } catch (CipherException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
