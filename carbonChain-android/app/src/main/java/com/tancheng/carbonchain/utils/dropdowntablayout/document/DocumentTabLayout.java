package com.tancheng.carbonchain.utils.dropdowntablayout.document;
...
public class DocumentTabLayout extends TabLayout {
    private String TAG = getClass().getSimpleName();
    private Context mContext;
    public DocumentTabLayout(Context context) {
        this(context, null);
    }

    public DocumentTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DocumentTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
    }

    private SparseArray sparseArray = new SparseArray<Integer>();
    private TextView mTabTextType;
    private ImageView mTabImageType;
    private TextView mTabTextAlbum;
    private View mTabTypeBg;
...
//            addTab(newTab().setText(titleList.get(i).get(0)));//默认=取第一个作为首次加载显示的tab名字
    ...
    public View getTabView(final int position, final List<String> titleList){
        final View view = LayoutInflater.from(mContext).inflate(R.layout.custom_tab_layout_document_item_tab, null);
        ...
        if(titleList.size() == 1) {
            ...
            //左边 筛选 tab
            tabText.setText(getResources().getString(R.string.drop_down_tab_layout_document_text_pre) + titleList.get(0));
            mTabTextType = tabText;
            mTabImageType = tabImg;
            mTabTypeBg = tabBg;
            textAndImg.setOnClickListener(new OnClickListener() {
                @Override
    ...
    public void changeTabColor(int tabPosition){
        ...
        if(tabPosition == 0){
            mTabTextType.setTextColor(getResources().getColor(R.color.drop_down_tab_layout_text_color_true));
            mTabTextAlbum.setTextColor(getResources().getColor(R.color.drop_down_tab_layout_text_color_false));
            mTabImageType.setImageResource(R.mipmap.arrow_down_select_true);
            mTabTypeBg.setVisibility(View.VISIBLE);
            mTabAlbumBg.setVisibility(View.GONE);
        }else{
            mTabTextType.setTextColor(getResources().getColor(R.color.drop_down_tab_layout_text_color_false));
            mTabTextAlbum.setTextColor(getResources().getColor(R.color.drop_down_tab_layout_text_color_true));
            mTabImageType.setImageResource(R.mipmap.arrow_down_select_false);
            mTabTypeBg.setVisibility(View.GONE);
                ...
                public View getView(final int position, View convertView, ViewGroup parent) {
                    ...
                    view.setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            tabText.setText(getResources().getString(R.string.drop_down_tab_layout_document_text_pre) + titleList.get(position));
